...
export default function ClientAdminLoginPage() {
  ...
  const [email, setEmail] = useState('clientadmin@mpaas.com');
  const [password, setPassword] = useState('••••••••');
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (status === 'loading') return;
    const isClientAdminRole = user && ['client_admin', 'supervisor', 'qa_manager', 'operations_manager', 'viewer'].includes(user.role);
    if (isAuthenticated && isClientAdminRole) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Enforce client_admin related domains
    const isAuthorizedEmail = ['clientadmin', 'supervisor', 'qa', 'operations', 'viewer'].some(x => email.includes(x));
    if (!isAuthorizedEmail) {
      setError(isRtl ? 'البريد الإلكتروني المدخل غير مصرح به لبوابة إدارة العميل' : 'Only Client Admin, QA, Operations, and Supervisor credentials are authorized.');
      return;
    }
  ...
  return (
    <AuthLayout
      badge={isRtl ? 'بوابة إدارة عمليات العميل' : 'Client Operations Portal'}
      title={isRtl ? 'تسجيل دخول الإدارة' : 'Operations Sign In'}
      subtitle={isRtl ? 'أدخل بيانات الاعتماد الخاصة بك لإدارة البوتات والفرق الفنية' : 'Sign in to configure chatbot safety policies and schedule shift rosters.'}
    >
