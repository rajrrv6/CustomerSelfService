...
export function BotsTab() {
  ...
  const handleResetWizard = () => {
    ...
    setNewBotDomain('Retail');
    setNewBotLangs(['English', 'Arabic']);
    setNewBotPersonality('An empathetic financial chatbot helper.');
    setNewBotTone('Friendly');
    setNewBotEscalation('fallback_limit');
    setNewBotMultiBehavior('auto_detect');
    setNewBotChannels({ web: true, whatsapp: false, email: false, voice: false, instagram: false, messenger: false });
    setWhatsappNumber('+966 50 123 4567');
    setWebChatColor('#2563EB');
    setSipTrunkGateway('VoIP Gate AST/KSA');
    setSupportEmail('support@mpaas-tenant.com');
    setSelectedKB(['kb-custom']);
      ...
      >
          ...
          {wizardStep === 3 && (
            <div className="space-y-5 animate-fade-in">
              ...
              <div className="bg-slate-50 dark:bg-slate-950/40 p-4 border border-slate-150 dark:border-slate-850 rounded-2xl space-y-4">
                <h4 className="font-bold text-slate-900 dark:text-white border-b border-slate-100 dark:border-slate-905 pb-1">{lang === 'ar' ? 'معايير الاتصال المفعلة' : 'Active Channel Metadata Settings'}</h4>
          ...
          {wizardStep === 5 && (
            <div className="space-y-4 animate-fade-in">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                {/* Chat window */}
                <div className="lg:col-span-2 bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-850 rounded-2xl flex flex-col justify-between overflow-hidden h-96">
                  {/* messages list */}
                ...
                <div className="bg-slate-50 dark:bg-slate-950/40 p-4 border border-slate-150 dark:border-slate-850 rounded-2xl space-y-3">
                  ...
                  <div className="space-y-2 text-[11px]">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-slate-650 dark:text-slate-350 truncate max-w-44" title={embeddingModel}>{embeddingModel}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-slate-850 dark:text-white truncate max-w-44">
                      ...
                      {['development', 'staging', 'production'].map((env) => {
                        ...
                        return (
                          <button
                            ...
                            className={`py-2 px-1 border font-bold uppercase rounded-xl transition-colors focus:outline-none text-[10px] ${
                              isSelected 
                      ...
                      {[
                        ...
                        return (
                          <button
                            ...
                            className={`py-2 px-1 border font-bold rounded-xl transition-colors focus:outline-none text-[10px] ${
                              isSelected 
        ...
        <div className="flex justify-between items-center pt-4 border-t border-slate-100 dark:border-slate-800 mt-4">
          <button
            ...
            className="flex items-center gap-1 px-4 py-2 border border-slate-200 dark:border-slate-800 rounded-xl font-bold hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-350 disabled:opacity-40"
          >
