...
import { EscalationMatrix } from '../operations/EscalationMatrix';
...
export function ClientSettingsWorkspace() {
  ...
  // Tab State
  const [activeTab, setActiveTab] = useState<'general' | 'sla' | 'escalation'>('general');
  ...
  return (
    <div className="space-y-6 animate-fade-in text-xs font-semibold text-slate-800 dark:text-slate-200" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      {/* Tab Selectors */}
      <div className="flex bg-slate-100 dark:bg-slate-900 p-0.5 rounded-xl border border-slate-200/40 dark:border-slate-855 font-mono text-[10px] font-bold w-fit select-none">
        {[
          { id: 'general', label: isAr ? 'العامة والهوية البصرية' : 'General & Branding' },
          { id: 'sla', label: isAr ? 'سياسات الـ SLA' : 'SLA Policy Rules' },
          { id: 'escalation', label: isAr ? 'مصفوفة التصعيد' : 'Escalation Matrix' },
        ].map((tab) => (
          <button
            key={tab.id}
            type="button"
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <OperationalCard className="p-5 space-y-4">
              ...
              <div className="space-y-3.5">
                <div className="space-y-1">
                  <label htmlFor="tenant-brand-name" className="text-[10px] font-bold text-slate-500">{isAr ? 'اسم العلامة التجارية:' : 'Tenant Brand Name:'}</label>
                  <input
                ...
                <div className="space-y-1">
                  <label htmlFor="brand-logo-url" className="text-[10px] font-bold text-slate-500">{isAr ? 'رابط ملف الشعار LOGO (URL):' : 'Branding Logo URL:'}</label>
                  <input
                    id="brand-logo-url"
                    type="text"
                ...
                <div className="grid grid-cols-2 gap-4.5">
                  <div className="space-y-1">
                    <label htmlFor="brand-ui-theme" className="text-[10px] font-bold text-slate-500">{isAr ? 'مظهر واجهة المشرف:' : 'Branding UI Theme:'}</label>
                    <select
                      id="brand-ui-theme"
                  ...
                  <div className="space-y-1">
                    <label htmlFor="primary-accent-color" className="text-[10px] font-bold text-slate-500">{isAr ? 'لون اللمسات الأساسية HSL:' : 'Primary Accent Color:'}</label>
                    <div className="flex gap-2">
                      <input
              ...
              <div className="space-y-3.5">
                <div className="space-y-1">
                  <div className="flex justify-between items-center text-[10px] font-bold text-slate-500">
                    ...
                    <span className="font-mono text-slate-855 dark:text-white">{localSettings.standardWait}s</span>
                  </div>
                  <input
                    type="range"
                    aria-label="Standard Client Queue Timeout"
                    min="30"
                    max="300"
                    step="10"
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center text-[10px] font-bold text-slate-500">
                    ...
                    <span className="font-mono text-slate-855 dark:text-white">{localSettings.vipWait}s</span>
                  </div>
                  <input
                    type="range"
                    aria-label="VIP Client Queue Timeout"
                    min="15"
                    max="120"
                    step="5"
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center text-[10px] font-bold text-slate-500">
                    ...
                    <span className="font-mono text-slate-855 dark:text-white">{localSettings.warnTimeout}s before breach</span>
                  </div>
                  <input
                    type="range"
                    aria-label="Warning Timer Trigger threshold"
                    min="5"
                    max="60"
                    step="5"
            ...
            <OperationalCard className="p-5 space-y-4">
              ...
              <div className="space-y-3.5">
                <div className="flex justify-between items-center py-1">
                  <div>
                    <h4 className="text-xs font-extrabold text-slate-855 dark:text-slate-100">{isAr ? 'احتواء المحادثات بالذكاء الاصطناعي' : 'AI Intent Deflection'}</h4>
                  ...
                  <label className="relative inline-flex items-center cursor-pointer shrink-0">
                    <input
                      type="checkbox"
                      aria-label="AI Intent Deflection toggle"
                ...
                <div className="flex justify-between items-center py-1 border-t border-slate-100 dark:border-slate-850/80 pt-3">
                  <div>
                    <h4 className="text-xs font-extrabold text-slate-855 dark:text-slate-100">{isAr ? 'مزامنة البحث الذكي RAG' : 'Vector RAG Policy Search'}</h4>
                  ...
                  <label className="relative inline-flex items-center cursor-pointer shrink-0">
                    <input
                      type="checkbox"
                      aria-label="Vector RAG Policy Search toggle"
                ...
                <div className="flex justify-between items-center py-1 border-t border-slate-100 dark:border-slate-850/80 pt-3">
                  <div>
                    <h4 className="text-xs font-extrabold text-slate-855 dark:text-slate-100">{isAr ? 'إعادة محاولة الاتصال بالشبكات' : 'High Reliability API Retry'}</h4>
                  ...
                  <label className="relative inline-flex items-center cursor-pointer shrink-0">
                    <input
                      type="checkbox"
                      aria-label="High Reliability API Retry toggle"
                ...
                <div className="flex justify-between items-center py-1 border-t border-slate-100 dark:border-slate-850/80 pt-3">
                  <div>
                    <h4 className="text-xs font-extrabold text-slate-855 dark:text-slate-100">{isAr ? 'تفصيل سجلات الويب للتحليل' : 'Webhook Payload Audits'}</h4>
                  ...
                  <label className="relative inline-flex items-center cursor-pointer shrink-0">
                    <input
                      type="checkbox"
                      aria-label="Webhook Payload Audits toggle"
            ...
            <OperationalCard className="p-5 space-y-4">
              ...
              <div className="space-y-3.5">
                <div className="space-y-1">
                  <label htmlFor="default-portal-lang" className="text-[10px] font-bold text-slate-500">{isAr ? 'اللغة الافتراضية للمنصة:' : 'Default Portal Language:'}</label>
                  <select
                    id="default-portal-lang"
      ...
      {activeTab === 'escalation' && <EscalationMatrix lang={lang} />}
          ...
          <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-3xl p-6 max-w-sm w-full space-y-4 shadow-xl">
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-2 text-amber-500">
                ...
                <h4 className="font-bold text-sm text-slate-805 dark:text-white">
