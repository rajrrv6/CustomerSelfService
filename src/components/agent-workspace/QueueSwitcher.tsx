  ...
  return (
    ...
    >
      ...
      <div
        id="queue-listbox"
        role="listbox"
        className={`absolute top-full left-0 right-0 mt-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-xl z-30 divide-y divide-slate-100 dark:divide-slate-800 ${
          isOpen ? 'block' : 'hidden'
        }`}
      >
        {queues.map((q) => (
          ...
          >
            <span className="truncate me-2">{q.name}</span>
