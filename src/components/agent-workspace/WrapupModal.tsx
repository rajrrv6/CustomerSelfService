...
export function WrapupModal({ isOpen, onClose, onResolve, slaStatus }: WrapupModalProps) {
  ...
  const slaLabels = {
    within_sla: { bg: 'bg-emerald-50 text-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-400 border-emerald-200/50 dark:border-emerald-800/30', label: 'WITHIN SLA TARGET' },
    warning: { bg: 'bg-amber-50 text-amber-800 dark:bg-amber-955/30 dark:text-amber-400 border-amber-200/50 dark:border-amber-800/30', label: 'SLA WARNING STAGE' },
    breached: { bg: 'bg-rose-50 text-rose-800 dark:bg-rose-950/30 dark:text-rose-400 border-rose-200/50 dark:border-rose-800/30', label: 'SLA BREACHED' }
  };

  const activeSla = slaLabels[slaStatus] || slaLabels.within_sla;

  return (
    <ModalWrapper isOpen={isOpen} onClose={onClose} title="Resolve Case & Disposition" maxWidthClass="max-w-sm">
      <form onSubmit={handleSubmit} className="space-y-4 text-xs font-semibold text-slate-800 dark:text-slate-200">
        ...
        <div>
          <label htmlFor="resolution-code" className="block text-[10px] font-bold text-slate-500 mb-1.5 uppercase font-mono tracking-wider">Resolution Code</label>
          <select
            id="resolution-code"
          ...
          >
            <option value="REFUND_PROCESSED" className="dark:bg-slate-900">REFUND_PROCESSED (Billing Approval)</option>
            <option value="ADDRESS_CORRECTED" className="dark:bg-slate-900">ADDRESS_CORRECTED (SAP Sync)</option>
            <option value="ROUTING_ACTIVATED" className="dark:bg-slate-900">ROUTING_ACTIVATED (Data Roaming)</option>
            <option value="TECH_ESCALATED" className="dark:bg-slate-900">TECH_ESCALATED (Level-3 Queue)</option>
            <option value="GENERAL_INFO" className="dark:bg-slate-900">GENERAL_INFO (Inquiry Answered)</option>
          </select>
        </div>

        <div>
          <label htmlFor="resolution-notes" className="block text-[10px] font-bold text-slate-500 mb-1.5 uppercase font-mono tracking-wider">Resolution / Close Notes</label>
          <textarea
            id="resolution-notes"
            required
        ...
        <div>
          <label htmlFor="follow-up-date" className="block text-[10px] font-bold text-slate-500 mb-1.5 uppercase font-mono tracking-wider">Schedule Follow-up (Optional)</label>
