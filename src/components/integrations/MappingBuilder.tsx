...
import { Layers, Plus, Save, Trash2, ArrowRight, ArrowLeft, RefreshCw, CheckCircle2 } from 'lucide-react';
...
export function MappingBuilder({ connector, onSave, isRtl = false }: MappingBuilderProps) {
  // Local DB Fields available for mapping
  const localDbFields = [
  ...
  return (
    <div className="bg-[#0b0f19]/35 border border-slate-850 rounded-3xl p-5 md:p-6 space-y-6 text-xs font-semibold text-slate-350">
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-[10px] text-slate-500 font-medium">Bind external attributes to Customer Portal schemas</p>
        </div>

        <button
          onClick={handleSave}
          className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold flex items-center gap-2 transition-all shadow-md shadow-blue-500/10 shrink-0"
        >
      ...
      {showSavedNotification && (
        <div className="flex items-center gap-2 p-3 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-xl animate-fade-in">
      ...
      {/* Add new mapping rule form */}
      <form onSubmit={handleAddRule} className="bg-slate-900/35 border border-slate-850 p-4 rounded-2xl flex flex-col md:flex-row md:items-end gap-4">
        <div className="flex-1 space-y-1.5">
          <label className="block text-slate-450 text-[10px] uppercase font-bold tracking-wide">
            {isRtl ? 'الحقل المحلي (Local System Field)' : 'Local System Field'}
          </label>
          <select
            value={selectedLocal}
            onChange={(e) => setSelectedLocal(e.target.value)}
            className="w-full px-3 py-2 bg-slate-950 border border-slate-850 rounded-xl text-slate-300 focus:outline-none focus:border-blue-500 font-medium"
          >
        ...
        <button
          ...
          className="py-2 px-4 bg-slate-900 border border-slate-800 text-slate-300 hover:text-white rounded-xl font-bold flex items-center justify-center gap-1.5 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
        >
      ...
      <div className="space-y-3">
        <h4 className="text-[10px] uppercase text-slate-500 tracking-wider font-bold">Active Mapped Parameters ({mappings.length})</h4>
        
        {mappings.length === 0 ? (
          <div className="text-center py-8 bg-slate-900/10 border border-dashed border-slate-850 rounded-2xl text-slate-500">
          ...
          <div className="border border-slate-850 bg-slate-950/20 rounded-2xl overflow-hidden divide-y divide-slate-850">
            {mappings.map(map => {
              ...
              return (
                <div key={map.localField} className="flex justify-between items-center p-3.5 hover:bg-slate-900/10 transition-colors">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-6">
                    <span className="font-mono text-white text-[10px] font-bold py-1 px-2.5 bg-slate-900 rounded-lg border border-slate-800">
                      {map.localField}
                    </span>
                    ...
                    <div>
                      ...
                      <span className="text-[9px] text-slate-500 font-medium ml-2 font-sans">
                        ({externalLabel})
                      </span>
                    </div>
                  </div>
