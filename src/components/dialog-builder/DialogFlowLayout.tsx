...
export function DialogFlowLayout() {
  ...
  return (
    <div
      className="flex flex-col h-[calc(100vh-80px)] border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden bg-white dark:bg-slate-900 shadow-sm"
      dir={lang === 'ar' ? 'rtl' : 'ltr'}
    >
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        {/* Simulator console view (Right sidebar) */}
        <WorkflowSimulator
          isRunning={isRunning}
      ...
      <div className="bg-slate-50 dark:bg-slate-950 px-6 py-3 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center text-[10px] font-mono text-slate-400 shrink-0">
        <span>mPaaS AI Workflow Engine: v1.8.0-stable</span>
        <div className="flex gap-2.5">
          <button
            onClick={handleLoadFlow}
            className="px-3 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 hover:bg-slate-50 rounded-lg text-slate-700 dark:text-slate-350"
          >
            Load Cache Draft
          </button>
          <button
