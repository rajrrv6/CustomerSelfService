...
export function WorkflowSimulator({
  ...
  return (
    <div className="w-80 border-l border-slate-200 dark:border-slate-800 bg-[#111827] text-white h-full flex flex-col shrink-0 text-xs font-semibold select-none shadow-xl">
      {/* Header */}
      ...
      <div className="flex-1 overflow-hidden relative flex flex-col">
        {activeView === 'chat' ? (
          <div className="flex-1 overflow-y-auto p-4 space-y-3 flex flex-col">
            {chatHistory.length === 0 && (
              <div className="my-auto text-center space-y-3 px-4">
                <HelpIcon className="w-8 h-8 mx-auto text-slate-650" />
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-1.5 font-mono text-[9px] leading-relaxed text-slate-350">
            {logs.map((log, idx) => (
      ...
      {isRunning && !waitingForInput && (
        <div className="p-3 bg-slate-950 border-t border-slate-900 flex justify-between items-center">
          <span className="text-[10px] text-slate-450 font-mono">Step debugger active:</span>
          <button
            onClick={onStep}
            className="flex items-center gap-1 px-3 py-1.5 bg-blue-650 hover:bg-blue-750 text-white font-bold rounded-lg shadow"
          >
