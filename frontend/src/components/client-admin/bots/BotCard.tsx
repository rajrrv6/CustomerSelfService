...
export function BotCard({ bot, lang, t, canEdit, onEditPersona }: BotCardProps) {
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {bot.language.map((l) => (
            <span 
              key={l} 
              className="px-2 py-0.5 rounded-md bg-slate-55 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-[9px] font-bold uppercase tracking-wide text-slate-400 dark:text-slate-500"
            >
              {l}
            </span>
          ))}
      ...
      <div className="pt-4 border-t border-slate-100 dark:border-slate-800/80 space-y-2.5">
        <div className="grid grid-cols-2 gap-2 text-center text-xs">
          <div className="bg-slate-50 dark:bg-slate-950 p-2 rounded-xl border border-slate-100 dark:border-slate-900/50">
            <span className="text-[9px] text-slate-455 dark:text-slate-500 uppercase font-mono block">{t.clientAdmin.bots.deflection}</span>
            <span className="font-bold text-slate-850 dark:text-white font-mono">{bot.deflectionRate}%</span>
          </div>
          <div className="bg-slate-50 dark:bg-slate-950 p-2 rounded-xl border border-slate-100 dark:border-slate-900/50">
            <span className="text-[9px] text-slate-455 dark:text-slate-500 uppercase font-mono block">{t.clientAdmin.bots.activeChats}</span>
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={!canEdit}
            className={`flex-1 py-2 text-xs font-semibold bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 rounded-xl text-slate-700 dark:text-slate-350 transition-colors cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${!canEdit ? 'opacity-60 cursor-not-allowed' : ''}`}
