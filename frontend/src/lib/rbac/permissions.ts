...
export function getScreenTitle(screenId: string, t: TranslationKeys): string {
  ...
  const mapping: Record<string, string> = {
    ...
    customer_notifications: isAr ? 'التنبيهات' : 'Notifications',
    customer_live_support: isAr ? 'الدعم المباشر' : 'Live Support',
    customer_recent_activity: isAr ? 'النشاط الأخير' : 'Recent Activity',
    customer_favorites: isAr ? 'المفضلة' : 'Favorites',
    customer_system_status: isAr ? 'حالة النظام' : 'System Status',
    customer_settings: isAr ? 'الإعدادات' : 'Settings',
